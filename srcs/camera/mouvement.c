...
t_vec3	camera_move_x(t_engine *engine, double delta_time)
{
...
t_vec3	camera_move_z(t_engine *engine, double delta_time)
{
	if (engine->input.forward)
		return (vec3_add(engine->camera.position,
				vec3_multiply(engine->camera.rotation,
					MOUVEMENT_SPEED * delta_time)));
...
t_vec3	camera_move_y(t_engine *engine, double delta_time)
{
	if (engine->input.up)
		return (vec3_add(engine->camera.position,
				vec3_multiply(engine->camera.up,
					MOUVEMENT_SPEED * delta_time)));
...
void	camera_move(t_engine *engine)
{
	...
	delta_time = engine->frame_details.frame_time[0] / 1000.0f;
	if (engine->input.left || engine->input.right)
		engine->camera.position = camera_move_x(engine, delta_time);
	if (engine->input.forward || engine->input.backward)
		engine->camera.position = camera_move_z(engine, delta_time);
	if (engine->input.up || engine->input.down)
		engine->camera.position = camera_move_y(engine, delta_time);
	if (engine->input.left || engine->input.right
